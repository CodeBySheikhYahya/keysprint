...
const users: User[] = [];
const testResults: TypingTestResult[] = [];
...
async function takeTypingTest(user: User) {
...
function calculateAccuracy(testText: string, typedText: string): number {
...
async function viewTestResults(user: User) {
...
function getSampleText(difficulty: string): string {
    switch (difficulty) {
        case 'Easy':
